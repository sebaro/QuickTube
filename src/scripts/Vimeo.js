...
function search(channel, page, token) {
	...
	if (token) {
		if (channel) {
			url = 'https://api.vimeo.com/users/' + channel + '/videos?fields=name,link,pictures.sizes,user.name,user.uri&sizes=640x360&per_page=20';
		}
		else {
			url = 'https://api.vimeo.com/videos?fields=name,link,pictures.sizes,user.name,user.uri&sizes=640x360&per_page=20';
		}
		// Safe
		if (safeInput.currentText == 'Yes') {
			url += "&filter_mature=7";
		}
		else {
			url += "&filter_mature=255";
		}
		// Sort
		if (sortInput.currentText == 'Relevance') {
			...
			url += "&sort=alphabetical";
		}
		// Channel
		if (channel) {
			if (channelInput.currentText == 'Yes' && searchWidget.searchInput.text) {
		...
		request.onsuccess.connect(function(result) {
			if (!token) {
		...
		if (debug) {
			request.onerror.connect(function(result) {
